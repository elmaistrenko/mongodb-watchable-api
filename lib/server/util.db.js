...
async function insert({insert}) {
    ...
    if (Array.isArray(insert))
        return this.insertMany(insert.map(mapper));
    return (await this.insertOne(mapper(insert))).result;
}

async function update({update, match: matchRaw, options: {isOne}}) {
    const match = transformId(matchRaw);
    const u = {
        ...update,
...
async function del({del: matchRaw, options: {isOne}}) {
    const match = transformId(matchRaw);
    return (await this[isOne ? 'updateOne' : 'updateMany'](match, {$set: {isDeleted: true}})).result;
}
